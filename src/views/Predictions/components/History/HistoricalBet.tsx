...
const HistoricalBet: React.FC<BetProps> = ({ bet }) => {
  ...
  const { targetRef, tooltip, tooltipVisible } = useTooltip(
    <>
      ...
      <Text>
        {t(
          'The Locked Price & Closed Price are exactly the same (within 8 decimals), so neither side wins. All funds entered into UP and DOWN positions will go to the weekly GOL burn.',
        )}
      </Text>
    </>,
    { placement: 'right' },
